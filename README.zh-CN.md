...
| **OpenCode** *（预发布 `v1.3.15-pre.2`）* | `opencode-plugin/` | 原生插件，已用 OpenCode 1.18.3 验证。Boot 走 `experimental.chat.system.transform`，prompt Recall 走 `chat.message`，原生 `lore_*` 工具不会带 MCP 前缀。 |
...
# 只安装 OpenCode 预发布版（v1.3.15-pre.2）
