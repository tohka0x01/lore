...
const DICT: Record<'zh' | 'en', Record<string, string>> = {
  zh: {
    ...
    'View LLM': '视图 LLM',
    'Boot URIs': '启动 URI 数',
    'Default (follow /settings)': '默认（跟随 /settings）',
    'raw_plus_lex_damp — raw score + lex damping · recommended · long-query safe': 'raw_plus_lex_damp — 原始分+lex长压 · 推荐 · 抗长 query',
    'raw_score — raw score sum · most faithful · quality=score': 'raw_score — 原始分相加 · 最诚实 · 质量=分数',
    'normalized_linear — normalized ranks · old default · long-query inflation': 'normalized_linear — 排名归一化 · 旧默认 · 长 query 虚高',
    'weighted_rrf — weighted rank fusion · uses path weights · 0-0.3 score': 'weighted_rrf — 加权 rank 融合 · 用路径权重 · 0-0.3 分',
    'rrf — rank fusion · 0-0.2 score · ranking only': 'rrf — rank 融合 · 0-0.2 分 · 只看排名',
    'max_signal — strongest signal wins · multi-path boost · tolerant': 'max_signal — 取最强信号 · 多路径加分 · 宽容',
    'cascade — signal tiers · exact>gs>dense · can exceed 1.0': 'cascade — 信号分级 · exact>gs>dense · 可超 1.0',
    'dense_floor — semantic threshold · low cosine gets cut · aggressive': 'dense_floor — 语义阈值 · 余弦低砍掉 · 激进',
    'soft': '宽松',
    'hard': '严格',
    ...
    'Legacy': '旧版',
    'Ready, unsafe': '可用但不安全',
    'Waiting for enough shown/used samples': '等待足够的展示/采用样本',
    'Ready sample volume, blocked by overlap': '样本量已足够，但因分数重叠被阻止',
    'Analytics-backed candidate midpoint': '基于分析结果的候选中点',
    'Threshold gap': '阈值差',
    'Suggested minus current': '建议值减去当前值',
    'Separation gap': '分离差',
    'Used p25 minus unused shown p75': '采用样本 p25 减去未采用展示样本 p75',
    'Display threshold analysis': '展示阈值分析',
    'Insufficient data': '数据不足',
    'Ready, unsafe to apply': '可复核，但暂不宜直接应用',
    'Ready to review': '可进入复核',
    'Threshold basis · insufficient data': '数据不足',
    'Threshold basis · midpoint used p25 unused shown p75': '采用样本 p25 与未采用展示样本 p75 的中点',
    'Threshold basis · used p25 minus margin': '采用样本 p25 减去安全边距',
    'Threshold basis · avg used minus margin': '采用样本平均值减去安全边距',
    'Basis': '依据',
    'Shown candidates': '展示候选',
    'Used candidates': '采用候选',
    'Unused shown': '未采用的展示候选',
    'Unused shown p75': '未采用展示样本 p75',
    'Used p25': '采用样本 p25',
    'Used median': '采用样本中位数',
    'Runtime min_display_score': '运行时 min_display_score',
    'Analytics suggestion': '分析建议值',
    'By source': '按来源',
    'No source-specific threshold samples yet.': '暂无按来源划分的阈值样本。',
    'Threshold guidance is waiting for enough shown and used candidates. Keep collecting recall events before changing the default display threshold.':
      '阈值建议仍在等待足够的展示与采用候选样本。在调整默认展示阈值前，请继续积累召回事件。',
    'Ready sample volume, unsafe execution': '样本量已足够，但当前执行不安全',
    'This window has enough shown and used candidates to compute a suggestion, but the current score overlap is still unsafe. A negative separation gap means used low-tail candidates are scoring below unused shown high-tail candidates, so directly raising the runtime threshold would likely hide memories that were actually used.':
      '当前窗口已有足够的展示与采用候选来计算建议值，但现有分数重叠仍然不安全。若分离差为负，表示已采用候选的低尾分数低于未采用展示候选的高尾分数，因此直接提高运行时阈值很可能会隐藏掉实际被用到的记忆。',
