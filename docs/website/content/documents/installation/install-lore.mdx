...
The native plugin also suppresses duplicate Lore MCP imports at runtime. This covers compatibility plugins that import Claude Code MCP entries such as `lore` or `lore:lore`, without modifying Claude Code or third-party configuration files. Set `LORE_OPENCODE_ALLOW_MCP=1` before starting OpenCode only when you explicitly need the legacy MCP fallback alongside the native plugin.
