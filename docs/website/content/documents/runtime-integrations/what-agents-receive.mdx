...
| --- | --- | --- |
| Boot memory | `session.start` through `/api/lifecycle/event` or `lore_boot` | stable baseline: shared agent rules, user preferences, and channel-specific rules |
| Recall candidates | `prompt.submit` through `/api/lifecycle/event` | prompt-specific memory leads with session/query identifiers |
